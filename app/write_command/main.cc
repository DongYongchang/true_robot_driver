...
/*  同步RPC  char* argv[] */
void SendRpcsy(std::shared_ptr<cpp_rpc::CPPClient>  client, const std::vector<std::string>& cmd_cmd, int outim_num, int sleep_num) {
...
/*  异步RPC  */
void SendRpcAsy(std::shared_ptr<cpp_rpc::CPPClient>  client, const std::vector<std::string>& cmd_cmd, int outim_num, int wait_num) {
    for (const auto& cmd : cmd_cmd) {
...
int main() {
    ...
    std::shared_ptr<cpp_rpc::CPPClient> client = std::make_shared<cpp_rpc::CPPClient>("192.168.11.11", 5868);
    std::cout << "Connected: " << std::endl;

    // 发送初始化指令
    SendRpcsy(client, init_cmds, 500, 100);  //同步rpc (连接client,指令,out time, sleep time)

    // 主循环发送运动指令
    while (1) {

        SendRpcsy(client, motion_cmds, 50000, 1000);
        //SendRpcAsy(client, motion_cmds,10000,500);  //异步rpc (连接client,指令,out time,sleep time)
    }
